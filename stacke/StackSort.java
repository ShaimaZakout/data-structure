package LabDataStructure.stacke;
...
public class StackSort {
    ...
    VectorStack<Integer> lowerValue=new VectorStack();
     VectorStack<Integer> upperValue=new VectorStack();
      
        for (int i = 0; i < data.length; i++) {// هنا اضافة الداتا الى الستاك
          // upperValue.push(data[i]);
          int check=data[i];
           while(!lowerValue.isEmpty()&&lowerValue.peek()>check){ //بدي اتاكد انه الي باخد منه  مش فاضي
          int move=lowerValue.pop();
          upperValue.push(move);
        }  //lowerValue.push(check);  ???
          while(!upperValue.isEmpty()&&upperValue.peek()<check){
          int move=upperValue.pop();
          lowerValue.push(move);
          } 
               upperValue.push(check); //يعني اذا كانوا كلا الستاك فاضيين بضيف على الابر
       
           
         } 
        while(!lowerValue.isEmpty()){
           upperValue.push(lowerValue.pop());
            }
          
         for (int i = 0; i < result.length; i++) { //هنا نعمل عكس لمحتويات الاري 
         result[i]=upperValue.pop();
         }
